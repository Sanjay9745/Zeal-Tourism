...
async function getGlobalVisa() {
  try {
    ...
    if (response.success) {
      const GlobalVisa = response.results; // Access `data.results` correctly

      // Render sections with fetched data
      $('#MainHeadpage').html(renderMainHeadpage(GlobalVisa));
      $('#VisaOptions').html(VisaOptions(GlobalVisa, GlobalVisa.pricing)); // Pass pricing if it exists
...
// Pass GlobalVisa as a parameter to access the images array and other properties
...
function VisaOptions(GlobalVisa, pricing) {
  return `
    <section id="Visa_Options">
      <div class="row">
        <div class="col-lg-8 col-xl-9 mb-4 mb-md-0 trending-slider">
                        ...
                        <div class="row">
                             <div class="col-12">
                                  <div class="row">
                                       <div class="col-md-9">
                                             <div class="card-body">
                                              <h6 class="card-title">${option.title}<span class="info-ouline curser-pointer" data-toggle="modal" data-target=".info">Info</span><h6>                                              
                                             </div>
                                       </div>
                                                ...
                                                <div class="warrapper">
                                                  <span class="h-name">Processing Type</span>
                                                   <select class="form-control ">
                                                    ${option.processType.map(type => `<option>${type}</option>`).join('')}
                                                   </select>
                                                  </div>
                                               </div>
                                               <div class="col-lg-6 mb-1">
                                                <div class="warrapper">
                                                  <span class="h-name">No. of Visa</span>
                                                   <select class="form-control ">
                                                     ${option.visaNo.map(visa => `<option>${visa}</option>`).join('')}
                                                   </select>
                                                  </div>
                                               </div>
                                            </div>
                              ...
                              <p class="card-text m-0 middle-line text-muted">${option.currency} ${option.price}</p>
                              <h5 class="price">${option.currency} ${(option.price * (1 - parseFloat(option.discountPercentage) / 100)).toFixed(2)}</h5>
                            </div>
                                            <div class="ml-2 pr-md-0 ">
                                              <div class="border p-2 rounded-ckecbox py-1">
                                                <div class="custom-control custom-checkbox ">
                                                  <input type="checkbox" class="custom-control-input" id="Select_${option._id}" name="example1">
                                  <label class="custom-control-label" for="Select_${option._id}">Select</label>
                                                </div>
                                              </div>
                                            </div>
                                           </div>
                                  ...
                                  <hr class="mt-1 mb-1">
                                 <p class="pl-2 mb-1">Credit Card Offer - Save up to <span class="theme-color font-weight-bold">${option.priceWithCurrency[0].currency} ${option.priceWithCurrency[0].discountPrice}</span> on Promocode ZEAL123</p>on Promocode ZEAL123</p>
                             </div>
                        </div>
                      </div>
                      </div>
          ...
          <div class="pagination py-2">
                ...
                <div class="col-lg-4 col-xl-3 mb-4  mb-md-0">
                    <div class="card border-0 rounded shadow Totalboxmain">
                         ...
                         <div class="panel panel-default">
                          <div class="card-header active" role="tab" id="A">
                            <a role="button" data-toggle="collapse" data-parent="#accordion" href=".A" aria-expanded="true" aria-controls="A">
                              <div class="row">
                                <div class="col">
                                   <h6><i class="fa-solid fa-angle-down"></i> 2 Units</h6>
                                </div>
                               <div class="col">
                                  <h6>AED ${pricing.packageCost.reduce((total, item) => total + item.amount, 0).toFixed(2)}</h6>
                                </div>
                             </div>
                           </a>
                          </div>
                          ...
                          <div class="card-header active" role="tab" id="B">
                            <a role="button" data-toggle="collapse" data-parent="#accordion" href=".B" aria-expanded="true" aria-controls="B">
                              <div class="row">
                               ...
                               <div class="col">
                                   <h6>${pricing.tax[0].currency} ${pricing.tax.reduce((total, item) => total + item.amount, 0).toFixed(2)}</h6>
                                </div>
                             </div>
                           </a>
                          </div>
                        ...
                        <div class="card-footer">
                          <div class="row">
                           ...
                           <div class="col">
                               <h6>AED ${(pricing.packageCost.reduce((total, item) => total + item.amount, 0) + pricing.tax.reduce((total, item) => total + item.amount, 0)).toFixed(2)}</h6>
                            </div>
                         </div>
                        </div>
                    </div>
                    ...
                    <div class="d-block mt-3">
                      <a href="Visa_review.html" role="button" class="btn btn-theam btn-block">PROCEED</a>
                      <button type="button" class="btn btn-theam-outer-line btn-block mt-3" id="Enquiry_visa">Enquire Now</button>
                    </div>
                </div>
           </div>
          </section>
